...
class Board:
    ...
    map = []  # Будет содержать карту точек кораблей, список строк (списков точек).
    ...
    free_dots = []  # Список свободных точек, используется лишь для ускорения размещения кораблей
    ...
    def __init__(self, hidenboard=False):
        ...
        for r in range(6):  # Заполняем список свободных для размещения кораблей точек [0-5][0-5]
            for c in range(6):
                self.free_dots.append((r, c))  # self.map[c][r]
